...
import . "github.com/noypi/graph/types"
...
type Path struct {
	stack       []morphism
	g           Graph // Optionally. A nil qs is equivalent to a morphism.
	baseContext context
}

// IsMorphism returns whether this Path is a morphism.
func (p *Path) IsMorphism() bool { return p.g == nil }
...
func StartPath(g Graph, nodes ...string) *Path {
	return &Path{
		stack: []morphism{
			isMorphism(nodes...),
		},
...
func PathFromIterator(g Graph, it VertexIter) *Path {
	return &Path{
		stack: []morphism{
			iteratorMorphism(it),
		},
...
func (p *Path) Morphism() graph.ApplyMorphism {
	return func(g Graph, it VertexIter) VertexIter {
		i := it.Clone()
		ctx := &p.baseContext
		for _, m := range p.stack {
			i, ctx = m.Apply(g, i, ctx)
		}
		return i
	}
}
